...
def apply_distributed(opt):
    if opt['rank'] == 0:
        # Get the IP address in a platform-independent way
        try:
            # Most reliable cross-platform way to get IP
            s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            # This doesn't actually establish a connection
            s.connect(("8.8.8.8", 80))
            master_address = s.getsockname()[0]
            s.close()
        except:
...
def init_distributed(opt):
    ...
    # set up device
    if not opt['CUDA']:
        # Force world_size to 1 when using CPU
        opt['world_size'] = 1
